...
class DataLoader(object):
    ...
    def make_batching_dict(self):
        ...
        smp_char_labels = pad_sequences(
            self.batch_labels,
            maxlen=max(self.batch_label_length),
            dtype="int32",
            padding="post",
            value=self.Char_padding_id,
        )
        ...
        smp_trans_text = pad_sequences(
            self.batch_word_text,
            maxlen=max(self.batch_word_text_length),
            dtype=object,
            padding="post",
            value=" ",
        )
                ...
                with open(inp_file) as f:
                    for line in f:
                        ...
                        if (
                            ...
                            or (mat.shape[0] < len(char_labels))
                            or (len(char_tokens) > self.max_label_len)
                        ):
                        ...
                        expect_len_of_labels = max(
                            max(self.batch_label_length, default=0), len(char_tokens)
                        )
                        ...
                        if (
                            total_labels_in_batch > self.max_batch_label_len
                            or len(self.batch_data) == self.max_batch_len
                        ):
                            # #==============================================================

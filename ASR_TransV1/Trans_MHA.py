...
##############################################################################################################
#-------------------------------------------------------------------------------------------------------------
class PositionwiseFeedForward_Fnet(nn.Module):
    """Implements position-wise feedforward sublayer. FFN(x) = GELU(xW1 + b1)W2 + b2 """

    def __init__(self, d_model, d_ff, dropout=0.1):
        super(PositionwiseFeedForward_Fnet, self).__init__()
        self.w_1 = nn.Linear(d_model, d_ff)
        self.w_2 = nn.Linear(d_ff, d_model)
        self.dropout = nn.Dropout(dropout)
        self.layer_norm = nn.LayerNorm(d_ff)
    ...
    def forward(self, x):
        ...
        output = F.gelu(self.layer_norm(self.w_1(x)))
        output = self.dropout(output)
        output = self.w_2(output)
        return output

##############################################################################################################
class MultiHeadAttention_Fnet(nn.Module):
    ''' Multi-Head Attention module '''
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1):
        super().__init__()
    ...
    def forward(self, q, k, v, mask=None):
        ...
        #output = output.transpose(1,2).contiguous().view(sz_b,-1, n_head*d_v)
        #output = self.fc(output)
        output_seq = torch.fft.fft(torch.fft.fft(input_seq, dim=-1), dim=-2).real

...
# =========================================================
def forword_and_update(
    trainflag, smp_no, model, optimizer, input, Word_target, accm_grad, clip_grad_norm
):
...
def train_val_model(**kwargs):
    ...
    if trainflag and args.spec_aug_flag and spec_aug_flag:
        smp_feat_mask = Spec_Aug(
            smp_feat,
            args.min_F_bands,
            args.max_F_bands,
            args.time_drop_max,
            args.time_window_max,
        )
    ...
    if trainflag:
        try:
            Decoder_out_dict, cost_cpu = forword_and_update(
                trainflag,
                smp_no,
                model,
                optimizer,
            ...
            if "CUDA out of memory" in str(e):
                ...
                print(
                    "The model in OOM condition",
                    "smp_no",
                    smp_no,
                    "batch size for the batch is:",
                    input.shape,
                )
        ...
        if OOM:
            ...
            print(
                "The model running under OOM condition",
                "smp_no",
                smp_no,
                "batch size for the batch is:",
                2,

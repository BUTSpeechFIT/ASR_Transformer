...
def main():
        ...
        Wfiles=open("weight_files/LIB_Trans_100M_12_6_512_8_2048_dr0.1_noclipgrad_IITM_finetune_accm4_v2_10K_SpecAug2",'r')
        for epoch,Wfile in enumerate(Wfiles.readlines()):
            Wfile=Wfile.strip()
            args.pre_trained_weight=str(Wfile)
            ...
            #============================================================
            #------------------------------------------------------------  
            dev_gen = DataLoader(files=glob.glob(args.data_dir + "dev_scp_dev_IITM"),
            ...
            for vl_smp in range(args.max_val_examples):
                ...
                if (val_examples >= args.max_val_examples):
                    break;
                #--------------------------------------                
                Val_Output_trainval_dict=train_val_model(smp_no=vl_smp,
                                                        args=args,
                                                        model = model,
                                                        optimizer = optimizer,
                                                        data_dict = B1,
    ...
    #==================================================================
            ...
            #================================================================== 
            ####saving_weights 
            #ct="model_epoch_"+str(epoch)+"_sample_"+str(trs_no)+"_"+str(mean(L_val_cost))+"___"+str(mean(L_val_cost))+"__"+str(mean(Vl_CER))
            #print(ct)
            ...
            with open(args.weight_text_file,'a+') as weight_saving_file:
                print(Wfile, file=weight_saving_file)

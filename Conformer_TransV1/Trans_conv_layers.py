...
class Conv_2D_Layers(nn.Module):
        def __init__(self,args):
                ...
                #self.conv3=torch.nn.Conv2d(in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=self.kernel_size, stride=(1,self.stride), padding=1, dilation=1, groups=1, bias=True)
                #self.conv4=torch.nn.Conv2d(in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=self.kernel_size, stride=(1,self.stride), padding=1, dilation=1, groups=1, bias=True)
        ...
        def forward(self, input):
                ...
                #=====#=====>
                #CV3=F.relu(self.conv_dropout_layer(self.conv3(CV2)))
                #CV4=F.relu(self.conv_dropout_layer(self.conv4(CV3)))
...
#=============================================================================================================
class Conv_2D_4Layers(nn.Module):
        def __init__(self,args):
                super(Conv_2D_4Layers,self).__init__()
                self.input_size = int(args.input_size)
                ...
                ###two subsamling conv layers
                self.conv1 = torch.nn.Conv2d(in_channels=self.in_channels, out_channels=self.out_channels, kernel_size=self.kernel_size, stride=(self.stride,1), padding=1, dilation=1, groups=1, bias=True)
                self.conv2 = torch.nn.Conv2d(in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=self.kernel_size, stride=(self.stride,1), padding=1, dilation=1, groups=1, bias=True)
